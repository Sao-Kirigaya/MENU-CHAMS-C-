...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, hovered ? main_color : background_color, ImGui::GetIO().DeltaTime * 10.f);
...
bool ImGui::Checkbox(const char* label, bool* v, const char* hint, int* keybind)
{
    ...
    it_anim->second.circle_offset = ImLerp(it_anim->second.circle_offset, *v ? 20.f : 0.f, g.IO.DeltaTime * anim_speed);
    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? main_color : background_color, g.IO.DeltaTime * anim_speed);
    it_anim->second.circle_color = ImLerp(it_anim->second.circle_color, *v ? ImColor(0.294f, 0.961f, 0.259f, 1.0f) : ImColor(1.f, 1.f, 1.f, 1.f), g.IO.DeltaTime * anim_speed);
    it_anim->second.text_color_offset = ImLerp(it_anim->second.text_color_offset, *v ? ImColor(0.f, 0.f, 0.f, 0.0f) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * anim_speed);
    ...
    if (*v) {
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            window->DrawList->AddCircleFilled(it_anim->second.particle_pos[i], it_anim->second.particle_radius[i], ImColor(0.f, 0.f, 0.f, it_anim->second.particle_alpha[i]));
        }
        PopClipRect();
    }

    window->DrawList->AddRectFilled(total_bb.Max - ImVec2(60, 35), total_bb.Max - ImVec2(20, 15), ImColor(0.1f, 0.1f, 0.1f, 0.5f), 25);
...
bool ImGui::Tab(const char* icon, int* tab, int number)
{
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(50, 50));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, *tab == number ? main_color : ImColor(1.f, 1.f, 1.f, 0.f), g.IO.DeltaTime * anim_speed);
    ...
    PushFont(icon_font);
    window->DrawList->AddText(total_bb.Min + ImVec2(25 - CalcTextSize(icon).x / 2, 25 - CalcTextSize(icon).y / 2), GetColorU32(it_anim->second.text_color), icon);
    PopFont();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int value,  const char* hint)
{
    ...
    const ImRect rect_bb(total_bb.Max - ImVec2(130, 35), total_bb.Max - ImVec2(40, 15));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    if (IsMouseHoveringRect(rect_bb.Max + ImVec2(2, -20), rect_bb.Max + ImVec2(22, 0), true) && IsMouseClicked(ImGuiMouseButton_Left))
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, background_color, 1.f);

    window->DrawList->AddRect(rect_bb.Min, rect_bb.Max, second_color, 1.f);

    window->DrawList->AddRect(rect_bb.Max + ImVec2(2, -20), rect_bb.Max + ImVec2(22, 0), second_color, 1.f);
    window->DrawList->AddRect(rect_bb.Min - ImVec2(22, 0), rect_bb.Min - ImVec2(2, -20), second_color, 1.f);
    ...
    window->DrawList->AddText(rect_bb.Min + ImVec2(5 + it_anim->second.text_offset, (rect_bb.Max.y - rect_bb.Min.y) / 2 - CalcTextSize(preview_value).y / 2), GetColorU32(it_anim->second.preview_color), preview_value);
    window->DrawList->AddShadowCircle(rect_bb.Max - ImVec2(0, 10), 20.f, second_color, 45.f, ImVec2(0, 0));
    PopClipRect(); 

    PushFont(icon_font);
    window->DrawList->AddText(rect_bb.Max + ImVec2(12, -10) - CalcTextSize("4") / 2, GetColorU32(it_anim->second.text_color), "4");
    window->DrawList->AddText(rect_bb.Min - ImVec2(12, -10) - CalcTextSize("3") / 2, GetColorU32(it_anim->second.text_color), "3");
    window->DrawList->AddText(second_font, 18, total_bb.Min + ImVec2(15, 25), GetColorU32(it_anim->second.text_color), hint);
    PopFont();
    ...
    if (CalcTextSize(preview_value).x > 90)
        it_anim->second.text_offset = ImLerp(it_anim->second.text_offset, IsMouseHoveringRect(rect_bb.Min, rect_bb.Max) ? 85 - CalcTextSize(preview_value).x : 0.f, g.IO.DeltaTime * anim_speed / 2);
    else
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, const char* hint)
{
    ...
    if (IsMouseHoveringRect(rect_bb.Max + ImVec2(2, -20), rect_bb.Max + ImVec2(22, 0), true) && IsMouseClicked(ImGuiMouseButton_Left) && *current_item < items_count - 1)
        *current_item += 1;

    if (IsMouseHoveringRect(rect_bb.Min - ImVec2(22, 0), rect_bb.Min - ImVec2(2, -20), true) && IsMouseClicked(ImGuiMouseButton_Left) && *current_item > 0)
         *current_item -= 1;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect slider_bb(frame_bb.Min + ImVec2(15, 30), frame_bb.Max - ImVec2(15, 13));
    ...
    it_anim->second.grab_offset = ImLerp(it_anim->second.grab_offset, (grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2) - slider_bb.Min.x, g.IO.DeltaTime * (anim_speed * 3));
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, hovered ? ImColor(1.f, 1.f, 1.f, 1.f) : ImColor(0.6f, 0.6f, 0.6f, 1.f), g.IO.DeltaTime * anim_speed);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, background_color, style.FrameRounding);
    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, second_color, 1.f);

    PushClipRect(slider_bb.Min, slider_bb.Min + ImVec2(it_anim->second.grab_offset, slider_bb.Max.y - slider_bb.Min.y), true);

    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Min + ImVec2(it_anim->second.grab_offset, slider_bb.Max.y - slider_bb.Min.y), main_color, 1.f);

    for (int i = 0; i < 50; i++)
    {
        if (!frame_bb.Contains(it_anim->second.particle_pos[i]))
        {
            it_anim->second.particle_pos[i] = window->DC.CursorPos + ImVec2(frame_size.x / 2, -5);
            it_anim->second.particle_speed[i] = ImVec2(random_float(-8.000f, 8.000f), random_float(-8.000f, 8.000f));
            it_anim->second.particle_radius[i] = random_float(1.000f, 3.000f);
            it_anim->second.particle_alpha[i] = random_float(0.000f, 1.000f);
        }
        it_anim->second.particle_pos[i] += it_anim->second.particle_speed[i] * ImVec2(g.IO.DeltaTime * anim_speed, g.IO.DeltaTime * anim_speed);
        window->DrawList->AddCircleFilled(it_anim->second.particle_pos[i], it_anim->second.particle_radius[i], ImColor(1.f, 1.f, 1.f, it_anim->second.particle_alpha[i]));
    }
    PopClipRect();

    // Añadir el círculo con sombra
    float circle_x = slider_bb.Min.x + (slider_bb.Max.x - slider_bb.Min.x) * (*(float*)p_data - *(float*)p_min) / (*(float*)p_max - *(float*)p_min); // Ajustar la posición
    float circle_y = grab_bb.Max.y - 3; // Posición del círculo ligeramente más abajo
    ImVec2 p = ImVec2(circle_x, circle_y); // Posición del círculo
    float shadow_radius = 8.f; // Radio de la sombra
    float shadow_blur = 10.f; // Desenfoque de la sombra
    ImVec2 shadow_offset = ImVec2(0, 0); // Desplazamiento de la sombra

    window->DrawList->AddShadowCircle(p, shadow_radius, main_color_shadow, shadow_blur, shadow_offset);
    window->DrawList->AddCircleFilled(p, 6.f, (main_color), 30.f);
    window->DrawList->AddCircle(p, 6.5f, ImColor(0, 0, 0), 30.f); // Outline

    window->DrawList->AddText(frame_bb.Min + ImVec2(15, 5), GetColorU32(it_anim->second.text_color), label);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 15 - CalcTextSize(value_buf).x, frame_bb.Min.y + 5), GetColorU32(it_anim->second.text_color), value_buf);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        //window->DrawList->AddShadowRect(frame_bb.Min, frame_bb.Max, ImColor(0, 0, 0, 60), 35.f, ImVec2(0, 4), ImDrawFlags_RoundCornersAll | ImDrawFlags_ShadowCutOutShapeBackground, 5.f);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(0.f, 0.f, 0.f, 0.1f), style.FrameRounding);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            window->DrawList->AddText(ImVec2(frame_bb.Min.x + 15.f + style.ItemInnerSpacing.x, frame_bb.Min.y + size_arg.y / 2 - CalcTextSize(buf_display).y / 2), GetColorU32(it_anim->second.color_text), buf_display);
        }
    ...
    {
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + 15.f + style.ItemInnerSpacing.x, frame_bb.Min.y + size_arg.y / 2 - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.color_text), label);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, const char* hint)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(total_bb.Min.x + 255, pos.y + 10);
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(main_window->Pos + ImVec2(150 + picker_size, 0));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.07f, 0.07f, 0.07f, 1.0f));
        popup_opened = BeginPopup("picker", ImGuiWindowFlags_NoMove);
        if (popup_opened)
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                picker_size = ImLerp(picker_size, 125.f, g.IO.DeltaTime * 15.f);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 picker_pos = window->DC.CursorPos + ImVec2(20, 20);
    float square_sz = GetFrameHeight();
    float bars_width = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = picker_size; // Saturation/Value picking box
    ...
    ImVec2 wheel_center(picker_pos + ImVec2(sv_picker_size / 2, sv_picker_size / 2));
    ...
    if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        SetCursorScreenPos(wheel_center - ImVec2(sv_picker_size / 1.5f, sv_picker_size / 1.5f));

        InvisibleButton("hsv", ImVec2(sv_picker_size * 1.35f, sv_picker_size * 1.35f));
        if (IsItemActive())
        {
            ImVec2 initial_off = g.IO.MousePos - wheel_center;
            ImVec2 current_off = g.IO.MousePos - wheel_center;
            float initial_dist2 = ImLengthSqr(initial_off);
            if (initial_dist2 >= (wheel_r_outer + 1) * (wheel_r_outer + 1) * 1.4f && initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1) * 1.7f)
            {
            ...
            if (initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1) * 0.9f)
            {
    ...
    if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        it_anim->second.sv_cursor_pos.x = ImLerp(it_anim->second.sv_cursor_pos.x, ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size), picker_pos.x + 2, picker_pos.x + sv_picker_size - 2), io.DeltaTime * 12.f); // Sneakily prevent the circle to stick out too much
        it_anim->second.sv_cursor_pos.y = ImLerp(it_anim->second.sv_cursor_pos.y, ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * sv_picker_size), picker_pos.y + 2, picker_pos.y + sv_picker_size - 2), io.DeltaTime * 12.f);

        draw_list->AddCircle(wheel_center, 80.f, ImColor(0.07f, 0.07f, 0.07f, 1.00f), 60, 35.f);
        ...
        for (int n = 0; n < 6; n++)
        {
            ...
            draw_list->PathArcTo(wheel_center, (wheel_r_inner + wheel_r_outer) * 0.7f, a0, a1, 60);
        ...
        it_anim->second.cos_hue_angle = ImLerp(it_anim->second.cos_hue_angle, ImCos(H * 2.0f * IM_PI), io.DeltaTime * 12.f);
        it_anim->second.sin_hue_angle = ImLerp(it_anim->second.sin_hue_angle, ImSin(H * 2.0f * IM_PI), io.DeltaTime * 12.f);

        it_anim->second.sv_cursor_rad = ImLerp(it_anim->second.sv_cursor_rad, value_changed_sv ? 10.0f : 6.0f, io.DeltaTime * 12.f);

        draw_list->AddCircleFilled(it_anim->second.sv_cursor_pos, it_anim->second.sv_cursor_rad, user_col32_striped_of_alpha, 60);
        draw_list->AddCircle(it_anim->second.sv_cursor_pos, it_anim->second.sv_cursor_rad + 1, col_midgrey, 60);
        draw_list->AddCircle(it_anim->second.sv_cursor_pos, it_anim->second.sv_cursor_rad, col_white, 60);

        ImVec2 hue_cursor_pos(wheel_center.x + it_anim->second.cos_hue_angle * (wheel_r_inner + wheel_r_outer) * 0.7f, wheel_center.y + it_anim->second.sin_hue_angle * (wheel_r_inner + wheel_r_outer) * 0.7f);
        float hue_cursor_rad = value_changed_h ? wheel_thickness * 0.65f : wheel_thickness * 0.55f;
        int hue_cursor_segments = ImClamp((int)(hue_cursor_rad / 1.4f), 9, 32);

        draw_list->AddCircleFilled(hue_cursor_pos, hue_cursor_rad, hue_color32, 60);
        draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad + 1, col_midgrey, 60);
        draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad, col_white, 60);
